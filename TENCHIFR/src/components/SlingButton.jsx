...
export default function SlingButton({
  ...
  const padT = useMotionTemplate`translate3d(${px}px, ${py}px, 0)`;
  ...
  const launchDot = () => {
    ...
    dotRefs.current.forEach((dot, i) => {
      ...
      dot.animate(
        [
          { transform: `translate3d(${cx * wellR}px, ${cy * wellR}px, 0) scale(${scale})` },
          { transform: `translate3d(${cx * to - cy * drift}px, ${cy * to + cx * drift}px, 0) scale(${shrink})` }
        ],
        { duration, delay, easing: EASE_OUT, fill: 'none' }
      );
      dot.animate(
        [
          { opacity: 0.95, offset: 0 },
  ...
  const relaxIcon = () => {
    const icon = iconRef.current;
    if (!icon) return;
    icon.style.transition = reduce ? 'none' : 'transform 450ms cubic-bezier(0.16, 1, 0.3, 1)';
  ...
  const settle = v0 => {
    ...
    const damping = Math.max(18, 34 - recoil * 32);
  ...
  return (
    <span
      ref={rootRef}
      className={`group/root relative inline-block ${className}`}
    ...
    >
      ...
      >
        <g ref={fxRef} className="sling-button__tension" style={{ opacity: 0 }}>
          <path
            ...
            stroke={bandColor}
            strokeLinecap="round"
            strokeWidth={armed ? strokeWidth * 1.5 : strokeWidth}
            style={{
              transition: 'stroke-width 220ms cubic-bezier(0.16, 1, 0.3, 1)'
            }}
          />
          <path
            ref={hotRef}
        ...
        <circle
          fill={sent ? accentColor : wellColor}
      ...
      {Array.from({ length: count }, (_, i) => (
        <span
          ...
          className="pointer-events-none absolute top-1/2 left-1/2 rounded-full opacity-0"
          style={{
            width: DOT,
            height: DOT,
            marginLeft: -DOT / 2,
        ...
        >
          <span
            ...
            style={{
              ...
              boxShadow: held
                ? 'none'
                : '0 2px 8px -1px rgba(0, 0, 0, 0.18), 0 1px 3px -1px rgba(0, 0, 0, 0.12)'
            }}
          >
      ...
      <span id={hintId} className="sr-only">
        {tapSends ? 'Press Enter to send, or drag away and release.' : 'Drag away and release to activate.'}
      </span>
    </span>
  );
}
